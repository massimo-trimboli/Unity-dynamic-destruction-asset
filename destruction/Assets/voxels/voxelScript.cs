...
public class voxelScript : MonoBehaviour
{
    ...
    static void voxelise(GameObject targetObject, float sizeOfVoxels, voxeliseOptions voxeliseOption, ref List<GameObject> gridList)
    {
        ...
        //loop through grid
        voxelFillGrid(targetObject, ref gridList);

        //apply colors
        targetObject.GetComponent<vTextureScrpt>().applyColor(gridList);

        applyOptions(targetObject, ref gridList, voxeliseOption.ToString());
    }
    ...
    static void voxelFillGrid(GameObject target, ref List<GameObject> grid)
    {
    ...
    static void applyOptions(GameObject target, ref List<GameObject> voxels, string option)
    {
        if (option == "combine")
        {
            ...
            if (target.GetComponent<voxelScript>().hasRigidBody)
